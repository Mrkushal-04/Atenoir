...
export default function SuitsPage() {
    ...
    const suitProducts = getProductsBySubcategory("suits")

    const filteredProducts = useMemo(() => {
        let filtered = suitProducts.filter(product => {
        ...
        return filtered
    }, [suitProducts, searchQuery, sortBy, priceRange, selectedColors, selectedSizes, selectedCategories])

    const allColors = Array.from(new Set(suitProducts.flatMap(p => p.colors.map(c => c.name))))
    const allSizes = Array.from(new Set(suitProducts.flatMap(p => p.sizes)))
    const allCategories = Array.from(new Set(suitProducts.map(p => p.category)))
        ...
        <div className="min-h-screen bg-black/[0.96] text-white">
            <div className="container mx-auto px-6 py-16">
                <div className="mb-12">
                    ...
                    >
                        Discover our premium collection of suits for men and women
                    </motion.p>
                </div>
                    ...
                    <div className="flex-1">
                        <div className="mb-6">
                            <p className="text-gray-400">
                                Showing {filteredProducts.length} of {suitProducts.length} suits
                            </p>
                        </div>
                        ...
                        {filteredProducts.length === 0 && (
                            <div className="text-center py-16">
                                <Crown className="w-16 h-16 mx-auto text-gray-600 mb-4" />
                                <h3 className="text-xl font-semibold mb-2">No suits found</h3>
