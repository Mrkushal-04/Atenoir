...
export default function AccessoriesPage() {
    ...
    const accessoriesProducts = getProductsBySubcategory("accessories")

    const filteredProducts = useMemo(() => {
        let filtered = accessoriesProducts.filter(product => {
        ...
        return filtered
    }, [accessoriesProducts, searchQuery, sortBy, priceRange, selectedColors, selectedSizes, selectedCategories])

    const allColors = Array.from(new Set(accessoriesProducts.flatMap(p => p.colors.map(c => c.name))))
    const allSizes = Array.from(new Set(accessoriesProducts.flatMap(p => p.sizes)))
    const allCategories = Array.from(new Set(accessoriesProducts.map(p => p.category)))
                    ...
                    <div className="flex-1">
                        <div className="mb-6">
                            <p className="text-gray-400">
                                Showing {filteredProducts.length} of {accessoriesProducts.length} accessories
                            </p>
                        </div>
